...
### 3.4 FetchReprojectionNeighborhood：读取膨胀 MV 邻域
...
### 3.6 ReprojectAllPrevTextures：历史重投影
...
### 3.8 ProcessPrevAtomicTexturesSamples：Disocclusion 检测与补洞
...
### 3.10 随机抖动量化（Stochastic Quantization）
...
### 3.12 历史帧复活（Resurrection）— DecimateHistory 阶段
...
```
Pass               输入纹理 (来源)                         输出纹理 (去向)
──────────────────────────────────────────────────────────────────────────────────────────────────────────────
ClearPrevTextures  (无，仅常量参数)                        TSR.PrevAtomics
                                                          → DilateVelocity: PrevAtomicOutput (RW InterlockedMax)
                                                          → DecimateHistory: PrevAtomicTextureArray (R)


DilateVelocity     TSR.PrevAtomics (RW, ← ClearPrevTex)   ClosestDepthOutput
                   SceneDepthTexture (外部)                → DecimateHistory: ClosestDepthTexture (R)
                   SceneVelocityTexture (外部)             → RejectShading: ClosestDepthTexture (R)

                                                          R8Output slice[0]: DilateMask
                                                          → DecimateHistory: DilateMaskTexture (R)
                                                          R8Output slice[1]: DeviceZError
                                                          → DecimateHistory: DepthErrorTexture (R)
...
RejectShading      InputTexture (外部 SceneColor)         History.GuideArray
                   ...
                   IsMovingMaskTexture (R, ← Dilate)      InputSceneColorOutput
                   ClosestDepthTexture (R, ← Dilate)      → UpdateHistory: InputSceneColorTex (R)
                   ResurrectedHistoryGuideTex (R, ← Dec)  InputSceneColorLdrLumaOutput
                                                          → SpatialAntiAliasing: InputLdrLumaTex (R)
                                                          AntiAliasMaskOutput
